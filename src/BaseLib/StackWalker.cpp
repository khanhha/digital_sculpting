...
// #############################################################
StackWalker::StackWalker(DWORD dwProcessId, HANDLE hProcess, unsigned const nMaxDepth /*= 3*/)
{
    this->SetMaxDepth(nMaxDepth);
...
StackWalker::StackWalker(int options, LPCSTR szSymPath, DWORD dwProcessId, HANDLE hProcess, unsigned const nMaxDepth /*= 3*/)
{
    this->SetMaxDepth(nMaxDepth);
...
#error "Platform not supported!"
    ...
    for (frameNum = 0; frameNum < m_nMaxDepth; ++frameNum )
    {
        ...
        if (s.AddrPC.Offset != 0)
        {
            ...
            if (this->m_sw->pSGSFA(this->m_hProcess, s.AddrPC.Offset, &(csEntry.offsetFromSmybol), pSym) != FALSE)
            {
                ...
                {
                    strcpy_s(csEntry.undName, pSym->Name);
                    strcpy_s(csEntry.undFullName, pSym->Name);
                }
            }
            else
            {
...
void StackWalker::OnCallstackEntry(CallstackEntryType eType, CallstackEntry &entry, int tablevel)
{
    CHAR buffer[STACKWALK_MAX_NAMELEN];
...
void StackWalker::OnOutput(LPCSTR buffer, int tablevel)
{
    assert(tablevel>=0 && tablevel < (int)m_nMaxDepth);

    //if (tablevel == 0)
    //{
    //    m_stackSymbols += std::string(buffer);
    //    m_stackSymbols += '\n';
    //}
    //else
    if (tablevel) // Ignore first item, which is StackWalker::ShowCallstack()
    {
        m_stackSymbols[tablevel-1] = std::string(buffer);
        //std::string symbol = std::string("\t") + std::string(buffer) + "\n";
        //m_stackSymbols += symbol;
    }

    //if (tablevel)
    //    OutputDebugStringA(tabstring(tablevel));
    //OutputDebugStringA(buffer);
    //OutputDebugStringA("\n");

    //if (tablevel)
    //    BaseType::Util::printLogMessage(tabstring(tablevel), false);
    //BaseType::Util::printLogMessage(buffer, true);
}