	...
	mNumZThreads(program.GetNumZThreads())
{
	...
	for (auto const& sb : program.GetSBuffers())
	{
		...
		default:
			qCritical("Unexpected structured buffer option: " +
...
unsigned int Shader::GetConstantBufferSize(int handle) const
{
    ...
    qCritical("Invalid handle for object.");
    return 0;
}
...
unsigned int Shader::GetConstantBufferSize(std::string const& name) const
{
    ...
    qCritical("Cannot find object " /*+ name + "."*/);
    return 0;
}
...
bool Shader::IsValid(Data const& goal, ConstantBuffer* resource) const
{
    if (!resource)
    {
        qCritical("Resource is null.");
        return false;
    }

    if (goal.type != GT_CONSTANT_BUFFER)
    {
        qCritical("Mismatch of buffer type.");
        return false;
    }
    ...
    qCritical("Invalid number of bytes.");
    return false;
}
...
bool Shader::IsValid(Data const& goal, StructuredBuffer* resource) const
{
    ...
    {
        qCritical("Mismatch of GPU write flag.");
        return false;
    }
#endif
    ...
    {
        qCritical("Mismatch of counter type.");
        return false;
    }
#else
...
bool Shader::IsValid(Data const& goal, TextureSingle* resource) const
{
    ...
    {
        qCritical("Mismatch of texture type.");
        return false;
    }
    ...
    {
        qCritical("Mismatch of texture dimensions.");
        return false;
    }
