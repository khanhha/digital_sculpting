    ...
    std::shared_ptr<Texture2> const& texture)
{
    ...
    {
        mTranslate =
            std::make_shared<ConstantBuffer>(sizeof(EVector2), true);
        mColor =
			std::make_shared<ConstantBuffer>(sizeof(EVector4), true);
...
void TextEffect::SetColor(EVector4 const& color)
{
	EVector4* data = mColor->Get<EVector4>();
    *data = color;
}
