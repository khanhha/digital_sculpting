...
bool VertexFormat::Bind(VASemantic semantic, DFType type, unsigned int unit)
{
    if (0 <= mNumAttributes && mNumAttributes < VA_MAX_ATTRIBUTES)
    {
        ...
        if (semantic == VA_COLOR)
        {
            ...
            {
                qErrnoWarning("Invalid color unit.");
                return false;
            }
        }
        else if (semantic == VA_TEXCOORD)
        {
            ...
            {
				qErrnoWarning("Invalid texture unit.");
                return false;
            }
        }
        else
        {
            if (unit > 0)
            {
				qErrnoWarning("Invalid semantic unit.");
                return false;
            }
        }
	...
	qErrnoWarning("Exceeded maximum attributes.");
    return false;
}
    ...
    unsigned int& unit, unsigned int& offset) const
{
	...
	qErrnoWarning(std::string("Invalid index" + std::to_string(i) + ".").c_str());
    return false;
}
