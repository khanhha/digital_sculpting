    ...
    mHandle(handle)
{
    ...
    {
        qCritical("The program handle is invalid.");
    }
}
...
void GLSLReflection::GetComputeShaderWorkGroupSize(GLint &numXThreads, GLint& numYThreads, GLint& numZThreads) const
{
    ...
    GL::function().glGetProgramiv(mHandle, GL_COMPUTE_WORK_GROUP_SIZE, workGroupSize);
...
void GLSLReflection::Print(std::ofstream& ostr) const
{
	...
	ostr << "OpenGL version = " << GL::function().glGetString(GL_VERSION) << std::endl;
	ostr << "GLSL version = " << GL::function().glGetString(GL_SHADING_LANGUAGE_VERSION) << std::endl;
	ostr << "Vendor = " << GL::function().glGetString(GL_VENDOR) << std::endl;
	ostr << "Renderer = " << GL::function().glGetString(GL_RENDERER) << std::endl;
    ostr << std::endl;
...
void GLSLReflection::ReflectProgramInputs()
{
	...
	GL::function().glGetProgramInterfaceiv(mHandle, GL_PROGRAM_INPUT, GL_ACTIVE_RESOURCES,
        &numResources);
    if (numResources > 0)
    {
        ...
        for (int i = 0; i < numResources; ++i)
        {
			...
			GL::function().glGetProgramResourceiv(mHandle, GL_PROGRAM_INPUT, i,
                numProperties, properties, numProperties, nullptr, results);

            GLint numBytes = results[0] + 1;
            GLchar* name = new GLchar[numBytes];
			GL::function().glGetProgramResourceName(mHandle, GL_PROGRAM_INPUT, i, numBytes,
                nullptr, name);
...
{
    GLint numResources = 0;
	GL::function().glGetProgramInterfaceiv(mHandle, GL_PROGRAM_OUTPUT, GL_ACTIVE_RESOURCES,
        &numResources);
    if (numResources > 0)
    {
        ...
        for (int i = 0; i < numResources; ++i)
        {
			...
			GL::function().glGetProgramResourceiv(mHandle, GL_PROGRAM_OUTPUT, i,
                numProperties, properties, numProperties, nullptr, results);

            GLint numBytes = results[0] + 1;
            GLchar* name = new GLchar[numBytes];
			GL::function().glGetProgramResourceName(mHandle, GL_PROGRAM_OUTPUT, i, numBytes,
                nullptr, name);
            info.name = std::string(name);
            delete[] name;
...
{
    GLint numResources = 0;
	GL::function().glGetProgramInterfaceiv(mHandle, GL_UNIFORM, GL_ACTIVE_RESOURCES,
        &numResources);
    ...
    if (numResources > 0)
    {
        ...
        {
            Uniform info;
			GL::function().glGetProgramResourceiv(mHandle, GL_UNIFORM, i,
                numProperties, properties, numProperties, nullptr, results);

            GLint numBytes = results[0] + 1;
            GLchar* name = new GLchar[numBytes];
			GL::function().glGetProgramResourceName(mHandle, GL_UNIFORM, i, numBytes,
                nullptr, name);
            info.name = std::string(name);
            delete[] name;
            ...
            {
				info.location = GL::function().glGetUniformLocation(mHandle, info.name.c_str());
            }
            ...
            if (info.arraySize > 1)
            {
                ...
                if (index != std::string::npos)
                {
                    ...
                    if (index2 != std::string::npos)
                    {
                        ...
                        qCritical("Only single dimensional arrays supported in GLSL uniforms.");
                    }
                ...
                else
                {
                    ...
                    qCritical("Unexpected condition.");
                }
            }
...
{
    GLint numResources = 0;
	GL::function().glGetProgramInterfaceiv(mHandle, programInterface, GL_ACTIVE_RESOURCES,
        &numResources);
    ...
    if (numResources > 0)
    {
        ...
        for (int i = 0; i < numResources; ++i)
        {
			...
			GL::function().glGetProgramResourceiv(mHandle, programInterface, i,
                numProperties, properties, numProperties, nullptr, results);

            GLint numBytes = results[0] + 1;
            GLchar* name = new GLchar[numBytes];
			GL::function().glGetProgramResourceName(mHandle, programInterface, i, numBytes,
                nullptr, name);
            info.name = std::string(name);
            delete[] name;
            ...
            // To be sure the bufferBinding field is set correctly, use this approach.
			info.bufferBinding = GL::function().glGetProgramResourceIndex(mHandle, programInterface, info.name.c_str());
            ...
            if (numActiveVariables > 0)
            {
				...
				GL::function().glGetProgramResourceiv(mHandle, programInterface, i, 1,
...
{
    GLint numResources = 0;
	GL::function().glGetProgramInterfaceiv(mHandle, GL_ATOMIC_COUNTER_BUFFER,
    ...
    if (numResources > 0)
    {
        ...
        for (int i = 0; i < numResources; ++i)
        {
			...
			GL::function().glGetProgramResourceiv(mHandle, GL_ATOMIC_COUNTER_BUFFER, i,
                numProperties, properties, numProperties, nullptr, results);
    ...
    std::vector<SubroutineUniform>& subUniforms)
{
    ...
    if (numResources > 0)
    {
        ...
        for (int i = 0; i < numResources; ++i)
        {
            ...
            GLint numBytes = results[0] + 1;
            GLchar* name = new GLchar[numBytes];
			GL::function().glGetProgramResourceName(mHandle, programInterface, i,
...
{
    GLint numResources = 0;
	GL::function().glGetProgramInterfaceiv(mHandle, GL_BUFFER_VARIABLE, GL_ACTIVE_RESOURCES,
        &numResources);
    if (numResources > 0)
    {
        ...
        for (int i = 0; i < numResources; ++i)
        {
			...
			GL::function().glGetProgramResourceiv(mHandle, GL_BUFFER_VARIABLE, i,
                numProperties, properties, numProperties, nullptr, results);

            GLint numBytes = results[0] + 1;
            GLchar* name = new GLchar[numBytes];
			GL::function().glGetProgramResourceName(mHandle, GL_BUFFER_VARIABLE, i, numBytes,
                nullptr, name);
            info.name = std::string(name);
            delete[] name;
...
{
    GLint numResources = 0;
	GL::function().glGetProgramInterfaceiv(mHandle, GL_TRANSFORM_FEEDBACK_VARYING,
        GL_ACTIVE_RESOURCES, &numResources);
    if (numResources > 0)
    {
        mInputs.resize(numResources);
        ...
        for (int i = 0; i < numResources; ++i)
        {
			...
			GL::function().glGetProgramResourceiv(mHandle, GL_TRANSFORM_FEEDBACK_VARYING, i,
                numProperties, properties, numProperties, nullptr, results);

            GLint numBytes = results[0] + 1;
            GLchar* name = new GLchar[numBytes];
			GL::function().glGetProgramResourceName(mHandle, GL_TRANSFORM_FEEDBACK_VARYING,
...
void GLSLReflection::IntelWorkaround(std::string const& name, GLint results[])
{
    ...
    GLsizei maxCount = 0;
	GL::function().glGetProgramiv(mHandle, GL_ATTACHED_SHADERS, &maxCount);
    if (maxCount < 0)
    {
        return;
    }
    ...
    GLsizei count = 0;
	GL::function().glGetAttachedShaders(mHandle, maxCount, &count, shaders.data());
    if (count != maxCount)
    {
        return;
    }
    ...
    {
        GLint type = 0;
		GL::function().glGetShaderiv(shader, GL_SHADER_TYPE, &type);
        ...
        if (iter != mShaderTypeMap.end())
        {
            ...
            if (results[index] == 0)
            {
                ...
                GLint length = 0;
				GL::function().glGetShaderiv(shader, GL_SHADER_SOURCE_LENGTH, &length);
                if (length <= 0)
                {
                    return;
                }

                GLchar* rawSource = new GLchar[length];
				GL::function().glGetShaderSource(shader, length, nullptr, rawSource);
