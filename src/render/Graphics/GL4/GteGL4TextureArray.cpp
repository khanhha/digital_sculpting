...
bool GL4TextureArray::CopyGpuToCpu(unsigned int item, unsigned int level)
{
	...
	if (item >= numItems)
	{
		qWarning("Item for Texture is out of range");
		return false;
	}
...
bool GL4TextureArray::DoCopyCpuToGpu(unsigned int item, unsigned int level)
{
    ...
    if (item >= numItems)
    {
        qWarning("Item for TextureArray is out of range");
        return false;
    }
    ...
    if (level >= numLevels)
    {
        qWarning("Level for TextureArray is out of range");
        return false;
    }
    ...
    if ((nullptr == data) || (0 == numBytes))
    {
        qWarning("No source data for TextureArray level=" + level);
        return false;
    }
