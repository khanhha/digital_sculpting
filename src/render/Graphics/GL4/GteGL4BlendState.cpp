...
GEObject* GL4BlendState::Create(void*, GraphicsObject const* object)
{
...
void GL4BlendState::Enable(HGLRC)
{
    ...
    {
        GL::function().glEnable(GL_SAMPLE_COVERAGE);
    }
    else
    {
		GL::function().glDisable(GL_SAMPLE_COVERAGE);
    }
    ...
    else
    {
        ...
        if (target.enable)
        {
            GL::function().glEnable(GL_BLEND);
			GL::function().glBlendFuncSeparate(target.srcColor, target.dstColor,
                target.srcAlpha, target.dstAlpha);
			GL::function().glBlendEquationSeparate(target.opColor, target.opAlpha);
        }
        else
        {
			GL::function().glDisable(GL_BLEND);
        }
		GL::function().glColorMask(target.rMask, target.gMask, target.bMask, target.aMask);
		GL::function().glSampleMaski(0, mSampleMask);
    }

	GL::function().glBlendColor(mBlendColor[0], mBlendColor[1], mBlendColor[2], mBlendColor[3]);
}
