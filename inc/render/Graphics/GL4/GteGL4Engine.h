...
namespace gte
{
	...
	public:
		...
		virtual ~GL4Engine();
#if 0
		GL4Engine(HWND handle, bool pixelFormatKnown, bool saveDriverInfo = false);
		GL4Engine(bool saveDriverInfo = false);
	...
	private:
		...
		void Initialize(bool pixelFormatKnown, bool saveDriverInfo);
	...
	public:
		...
		bool Enable(std::shared_ptr<VisualEffect> const& effect);
		void Disable(std::shared_ptr<VisualEffect> const& effect);
		void DrawVertexBuffer(std::shared_ptr<VisualEffect> const& effect, std::shared_ptr<VertexBuffer> const& vbuffer, GLenum topology = GL_TRIANGLES);
		uint64_t DrawBuffer(std::shared_ptr<VertexBuffer> const& vbuffer, std::shared_ptr<IndexBuffer> const& ibuffer, std::shared_ptr<VisualEffect> const& effect);

		BufferUpdater GetBufferUpdater();
	};
