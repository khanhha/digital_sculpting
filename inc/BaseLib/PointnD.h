#ifndef BASELIB_POINTND_H
#define BASELIB_POINTND_H

// Windows defines minT and maxT as macros, which prevents us from using the
// type-safe versions from std::, as well as interfering with method defns.
// Also define NOminTmaxT, which prevents future bad definitions.
...
#ifndef likely
#  if !defined(__GNUC__) || (__GNUC__ == 2 && __GNUC_minTOR__ < 96)
#    define likely(x) (x)
#    define unlikely(x) (x)
#  else
...
public:
	...
	PointnD() { for (int i = 0; i < D; i++) v[i] = T(0); }
...
#define VEC_UNINITIALIZED ((void *) 0)
	...
	explicit PointnD(const T &x)
		{ for (int i = 0; i < D; i++) v[i] = x; }

	// Constructors for 2-4 arguments
	PointnD(const T &x, const T &y)
		{ VEC_STATIC_CHECK(D == 2); v[0] = x; v[1] = y; }
	PointnD(const T &x, const T &y, const T &z)
		{ VEC_STATIC_CHECK(D == 3); v[0] = x; v[1] = y; v[2] = z; }
	PointnD(const T &x, const T &y, const T &z, const T &w)
	...
	template <class S> explicit PointnD(const S &x)
	...
	// Member operators
	PointnD<D,T> &operator += (const PointnD<D,T> &x)
	{
		for (int i = 0; i < D; i++)
#pragma omp atomic
			v[i] += x[i];
...
#pragma omp atomic
	...
	PointnD<D,T> &operator *= (const T &x)
	{
		for (int i = 0; i < D; i++)
#pragma omp atomic
			v[i] *= x;
...
#pragma omp atomic
	...
	// Set each component to minT/maxT of this and the other vector
	PointnD<D,T> &minT(const PointnD<D,T> &x)
	{
#pragma omp critical
		for (int i = 0; i < D; i++)
			if (x[i] < v[i]) v[i] = x[i];
	...
	PointnD<D,T> &maxT(const PointnD<D,T> &x)
	{
#pragma omp critical
		for (int i = 0; i < D; i++)
			if (x[i] > v[i]) v[i] = x[i];
...
#pragma omp critical
	...
	PointnD<D,T> apply(T func(T)) const
	{
		PointnD<D,T> result(VEC_UNINITIALIZED);
		for (int i = 0; i < D; i++) result[i] = func(v[i]);
		return result;
	}
	PointnD<D,T> apply(T func(const T&)) const
	{
		PointnD<D,T> result(VEC_UNINITIALIZED);
		for (int i = 0; i < D; i++) result[i] = func(v[i]);
		return result;
	}
	PointnD<D,T> cshift(int n) const
	{
		PointnD<D,T> result(VEC_UNINITIALIZED);
		if (n < 0)
			n = (n % D) + D;
	...
	PointnD<D,T> shift(int n) const
	{
		if (abs(n) >= D)
			return PointnD<D,T>();
		PointnD<D,T> result; // Must be initialized to zero
		int start = n < 0 ? -n : 0;
		int stop = n > 0 ? D - n : D;
...
//typedef PointnD<3,float> vec;
//typedef PointnD<3,float> point;
//typedef PointnD<2,float> vec2;
//typedef PointnD<3,float> vec3;
typedef PointnD<4,float> Vector4Df;
//typedef PointnD<2,int> ivec2;
//typedef PointnD<3,int> ivec3;
//typedef PointnD<4,int> ivec4;


// Nonmember operators that take two Vecs
template <int D, class T>
static inline const PointnD<D,T> operator + (const PointnD<D,T> &v1, const PointnD<D,T> &v2)
{
	PointnD<D,T> result(VEC_UNINITIALIZED);
	for (int i = 0; i < D; i++)
		result[i] = v1[i] + v2[i];
...
// Dot product
template <int D, class T>
static inline const T operator ^ (const PointnD<D,T> &v1, const PointnD<D,T> &v2)
{
	T sum = v1[0] * v2[0];
	for (int i = 1; i < D; i++)
		sum += v1[i] * v2[i];
...
template <class T>
static inline const PointnD<3,T> operator % (const PointnD<3,T> &v1, const PointnD<3,T> &v2)
{
	return PointnD<3,T>(v1[1]*v2[2] - v1[2]*v2[1],
			v1[2]*v2[0] - v1[0]*v2[2],
			v1[0]*v2[1] - v1[1]*v2[0]);
}
#define CROSS %
...
template <int D, class T>
static inline bool operator == (const PointnD<D,T> &v1, const PointnD<D,T> &v2)
{
	for (int i = 0; i < D; i++)
		if (v1[i] != v2[i])
			return false;
...
// Unary operators
template <int D, class T>
static inline const PointnD<D,T> &operator + (const PointnD<D,T> &v)
{
	return v;
}
...
template <int D, class T>
static inline bool operator ! (const PointnD<D,T> &v)
{
	return v.empty();
}
...
template <int D, class T>
static inline const PointnD<D,T> operator * (const T &x, const PointnD<D,T> &v)
{
	PointnD<D,T> result(VEC_UNINITIALIZED);
	for (int i = 0; i < D; i++)
		result[i] = x * v[i];
...
template <int D, class T>
static inline const PointnD<D,T> operator * (const PointnD<D,T> &v, const T &x)
{
	PointnD<D,T> result(VEC_UNINITIALIZED);
	for (int i = 0; i < D; i++)
		result[i] = v[i] * x;
...
// iostream operators
template <int D, class T>
static inline std::ostream &operator << (std::ostream &os, const PointnD<D,T> &v)
...
template <int D, class T>
static inline std::istream &operator >> (std::istream &is, PointnD<D,T> &v)
{
	char c1 = 0, c2 = 0;
...
namespace std {
  template <int D, class T>
  static inline void swap(const PointnD<D,T> &v1, const PointnD<D,T> &v2)
  {
	for (int i = 0; i < D; i++)
		swap(v1[i], v2[i]);
  }
...
template <int D, class T>
static inline T faceforward(const PointnD<D,T> &N, const PointnD<D,T> &I,
			    const PointnD<D,T> &Nref)
{
	return ((Nref DOT I) < T(0)) ? N : -N;
}

template <int D, class T>
static inline T reflect(const PointnD<D,T> &I, const PointnD<D,T> &N)
{
	return I - (T(2) * (N DOT I)) * N;
}

template <int D, class T>
static inline T refract(const PointnD<D,T> &I, const PointnD<D,T> &N,
			const T &eta)
{
...
inline double cbrtd(double x)
{
	using namespace std; // For pow
	return (x < 0.0) ? -pow(-x, 1.0 / 3.0) : pow(x, 1.0 / 3.0);
}
inline long double cbrtld(long double x)
{
	using namespace std; // For pow
...
inline long double roundld(long double x)
{
...
inline double truncd(double x)
{
	return (x < 0.0f) ? double(int(x)) : double(int(x));
}
inline long double truncld(long double x)
{
...
// Squared length
template <int D, class T>
static inline const T len2(const PointnD<D,T> &v)
{
	T l2 = v[0] * v[0];
	for (int i = 1; i < D; i++)
		l2 += v[i] * v[i];
...
// Length
template <int D, class T>
static inline const T len(const PointnD<D,T> &v)
{
	using namespace std; // For sqrt
	return sqrt(len2(v));
}
...
// Squared distance
template <int D, class T>
static inline const T dist2(const PointnD<D,T> &v1, const PointnD<D,T> &v2)
{
	T d2 = sqr(v2[0]-v1[0]);
	for (int i = 1; i < D; i++)
		d2 += sqr(v2[i]-v1[i]);
...
// Distance
template <int D, class T>
static inline const T dist(const PointnD<D,T> &v1, const PointnD<D,T> &v2)
{
	using namespace std; // For sqrt
...
template <int D, class T>
static inline PointnD<D,T> normalize(PointnD<D,T> &v)
{
	T l = len(v);
	if (unlikely(l <= T(0))) {
		v[0] = T(1);
...
template <int D, class T>
static inline const T angle(const PointnD<D,T> &v1, const PointnD<D,T> &v2)
{
	using namespace std;
...
#define VEC_DECLARE_ONEARG(name) \
 template <int D, class T> \
 static inline PointnD<D,T> name(const PointnD<D,T> &v) \
 { \
	using namespace std; \
	PointnD<D,T> result(VEC_UNINITIALIZED); \
	for (int i = 0; i < D; i++) \
...
#define VEC_DECLARE_TWOARG(name) \
 template <int D, class T> \
 static inline PointnD<D,T> name(const PointnD<D,T> &v, const T &w) \
 { \
	using namespace std; \
	PointnD<D,T> result(VEC_UNINITIALIZED); \
	for (int i = 0; i < D; i++) \
 ...
 template <int D, class T> \
 static inline PointnD<D,T> name(const PointnD<D,T> &v, const PointnD<D,T> &w) \
 { \
	using namespace std; \
	PointnD<D,T> result(VEC_UNINITIALIZED); \
	for (int i = 0; i < D; i++) \
...
#define VEC_DECLARE_THREEARG(name) \
 template <int D, class T> \
 static inline PointnD<D,T> name(const PointnD<D,T> &v, const T &w, const T &x) \
 { \
	using namespace std; \
	PointnD<D,T> result(VEC_UNINITIALIZED); \
	for (int i = 0; i < D; i++) \
 ...
 template <int D, class T> \
 static inline PointnD<D,T> name(const PointnD<D,T> &v, const PointnD<D,T> &w, const PointnD<D,T> &x) \
 { \
	using namespace std; \
	PointnD<D,T> result(VEC_UNINITIALIZED); \
	for (int i = 0; i < D; i++) \
...
namespace std {
 VEC_DECLARE_TWOARG(minT)
 VEC_DECLARE_TWOARG(maxT)
}
...
// Let's do the same...
template <int D, class T>
static inline PointnD<D,T> abs(const PointnD<D,T> &v)
{
	return fabs(v);
}
