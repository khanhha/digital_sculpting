#ifndef SCULPT_VEC4_H
#define SCULPT_VEC4_H
...
VBVH_BEGIN_NAMESPACE
  ...
  template<typename T> __forceinline Vec4<T> smin(const Vec4<T>& a, const Vec4<T>& b) { return Vec4<T>(smin(a.x, b.x), smin(a.y, b.y), smin(a.z, b.z), smin(a.w, b.w)); }
  template<typename T> __forceinline Vec4<T> smax(const Vec4<T>& a, const Vec4<T>& b) { return Vec4<T>(smax(a.x, b.x), smax(a.y, b.y), smax(a.z, b.z), smax(a.w, b.w)); }
  ...
  template<typename T> __forceinline T reduce_min( const Vec4<T>& a ) { return smin(a.x, a.y, a.z, a.w); }
  template<typename T> __forceinline T reduce_max( const Vec4<T>& a ) { return smax(a.x, a.y, a.z, a.w); }
