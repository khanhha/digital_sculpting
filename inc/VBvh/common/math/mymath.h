#ifndef SCULPT_MYMATH_H
#define SCULPT_MYMATH_H
...
VBVH_BEGIN_NAMESPACE
    ...
    __forceinline                    int smin(int     a, int     b)                                       { return a < b ? a : b; }
    __forceinline                  int64 smin(int64   a, int64   b)                                       { return a < b ? a : b; }
    __forceinline                 size_t smin(size_t  a, size_t  b)                                       { return a < b ? a : b; }
#if !defined(__WIN32__)
    __forceinline                ssize_t smin(ssize_t a, ssize_t b)                                       { return a < b ? a : b; }
#endif
    __forceinline                  float smin(float   a, float   b)                                       { return a < b ? a : b; }
    __forceinline                 double smin(double  a, double  b)                                       { return a < b ? a : b; }
    template<typename T> __forceinline T smin(const T& a, const T& b, const T& c)                         { return smin(smin(a, b), c); }
    template<typename T> __forceinline T smin(const T& a, const T& b, const T& c, const T& d)             { return smin(smin(a, b), smin(c, d)); }
    template<typename T> __forceinline T smin(const T& a, const T& b, const T& c, const T& d, const T& e) { return smin(smin(smin(a, b), smin(c, d)), e); }

    __forceinline                    int smax(int     a, int     b)                                       { return a < b ? b : a; }
    __forceinline                  int64 smax(int64   a, int64   b)                                       { return a < b ? b : a; }
    __forceinline                 size_t smax(size_t  a, size_t  b)                                       { return a < b ? b : a; }
#if !defined(__WIN32__)
    __forceinline                ssize_t max(ssize_t a, ssize_t b)                                       { return a < b ? b : a; }
#endif
    __forceinline                  float smax(float   a, float   b)                                       { return a < b ? b : a; }
    __forceinline                 double smax(double  a, double  b)                                       { return a < b ? b : a; }
    template<typename T> __forceinline T smax(const T& a, const T& b, const T& c)                         { return smax(smax(a, b), c); }
    template<typename T> __forceinline T smax(const T& a, const T& b, const T& c, const T& d)             { return smax(smax(a, b), smax(c, d)); }
    template<typename T> __forceinline T smax(const T& a, const T& b, const T& c, const T& d, const T& e) { return smax(smax(smax(a, b), smax(c, d)), e); }

    template<typename T> __forceinline T clamp(const T& x, const T& lower = T(zero), const T& upper = T(one)) { return smax(smin(x, upper), lower); }
    template<typename T> __forceinline T clampz(const T& x, const T& upper) { return smax(T(zero), smin(x, upper)); }
    ...
    template<typename T> __forceinline T  sin2cos(const T& x)  { return sqrt(smax(T(zero), T(one) - x*x)); }
