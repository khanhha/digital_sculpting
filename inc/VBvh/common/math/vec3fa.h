#ifndef SCULPT_VEC3FA_H
#define SCULPT_VEC3FA_H
...
VBVH_BEGIN_NAMESPACE
  ...
  __forceinline const Vec3fa smin( const Vec3fa& a, const Vec3fa& b ) { return _mm_min_ps(a.m128,b.m128); }
  __forceinline const Vec3fa smax( const Vec3fa& a, const Vec3fa& b ) { return _mm_max_ps(a.m128,b.m128); }
  ...
  __forceinline float reduce_min(const Vec3fa& v) { return smin(v.x,v.y,v.z); }
  __forceinline float reduce_max(const Vec3fa& v) { return smax(v.x,v.y,v.z); }
  ...
  __forceinline float  length   ( const Vec3fa& a )                  { return std::sqrt(dot(a,a)); }
...
#else
  __forceinline const Vec3fa trunc(const Vec3fa& a) {
      return Vec3fa(a.x > 0 ? floorf(a.x) : ceilf(a.x),  a.y > 0 ? floorf(a.y) : ceilf(a.y), a.z > 0 ? floorf(a.z) : ceilf(a.z));
  }
