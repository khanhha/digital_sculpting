#ifndef SCULPT_VEC2_H
#define SCULPT_VEC2_H
...
VBVH_BEGIN_NAMESPACE
  ...
  template<typename T> __forceinline Vec2<T> smin(const Vec2<T>& a, const Vec2<T>& b) { return Vec2<T>(smin(a.x, b.x), smin(a.y, b.y)); }
  template<typename T> __forceinline Vec2<T> smax(const Vec2<T>& a, const Vec2<T>& b) { return Vec2<T>(smax(a.x, b.x), smax(a.y, b.y)); }
  ...
  template<typename T> __forceinline T reduce_min( const Vec2<T>& a ) { return smin(a.x, a.y); }
  template<typename T> __forceinline T reduce_max( const Vec2<T>& a ) { return smax(a.x, a.y); }
