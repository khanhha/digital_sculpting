#ifndef SCULPT_VEC3IA_H
#define SCULPT_VEC3IA_H
...
VBVH_BEGIN_NAMESPACE
  ...
  __forceinline int reduce_min(const Vec3ia& v) { return smin(v.x,v.y,v.z); }
  __forceinline int reduce_max(const Vec3ia& v) { return smax(v.x,v.y,v.z); }
...
#else
  __forceinline const Vec3ia smin( const Vec3ia& a, const Vec3ia& b ) { return select(lt_mask(a,b),a,b); }
  __forceinline const Vec3ia smax( const Vec3ia& a, const Vec3ia& b ) { return select(gt_mask(a,b),a,b); }
#endif
