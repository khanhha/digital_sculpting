#ifndef SCULPT_SSEF_H
#define SCULPT_SSEF_H
...
VBVH_BEGIN_NAMESPACE
  ...
  __forceinline const ssef smin( const ssef& a, const ssef& b ) { return _mm_min_ps(a.m128,b.m128); }
  __forceinline const ssef smin( const ssef& a, const float& b ) { return _mm_min_ps(a.m128,ssef(b)); }
  __forceinline const ssef smin( const float& a, const ssef& b ) { return _mm_min_ps(ssef(a),b.m128); }

  __forceinline const ssef smax( const ssef& a, const ssef& b ) { return _mm_max_ps(a.m128,b.m128); }
  __forceinline const ssef smax( const ssef& a, const float& b ) { return _mm_max_ps(a.m128,ssef(b)); }
  __forceinline const ssef smax( const float& a, const ssef& b ) { return _mm_max_ps(ssef(a),b.m128); }
...
__forceinline const ssef select(const int mask, const ssef& t, const ssef& f) {
 ...
 __forceinline const ssef select(const int mask, const ssef& t, const ssef& f) {
  ...
  __forceinline const ssef vreduce_min(const ssef& v) { ssef h = smin(shuffle<1,0,3,2>(v),v); return smin(shuffle<2,3,0,1>(h),h); }
  __forceinline const ssef vreduce_max(const ssef& v) { ssef h = smax(shuffle<1,0,3,2>(v),v); return smax(shuffle<2,3,0,1>(h),h); }
