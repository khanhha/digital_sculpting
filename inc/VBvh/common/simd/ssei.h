#ifndef SCULPT_SSEI_H
#define SCULPT_SSEI_H
...
VBVH_BEGIN_NAMESPACE
...
#if defined(__SSE4_1__)
  __forceinline const ssei smin( const ssei& a, const ssei& b ) { return _mm_min_epi32(a.m128, b.m128); }
  __forceinline const ssei smax( const ssei& a, const ssei& b ) { return _mm_max_epi32(a.m128, b.m128); }
#else
  __forceinline const ssei smin( const ssei& a, const ssei& b ) { return ssei(smin(a[0],b[0]),smin(a[1],b[1]),smin(a[2],b[2]),smin(a[3],b[3])); }
  __forceinline const ssei smax( const ssei& a, const ssei& b ) { return ssei(smax(a[0],b[0]),smax(a[1],b[1]),smax(a[2],b[2]),smax(a[3],b[3])); }
#endif

  __forceinline const ssei smin( const ssei& a, const int32&  b ) { return smin(a,ssei(b)); }
  __forceinline const ssei smin( const int32&  a, const ssei& b ) { return smin(ssei(a),b); }
  __forceinline const ssei smax( const ssei& a, const int32&  b ) { return smax(a,ssei(b)); }
  __forceinline const ssei smax( const int32&  a, const ssei& b ) { return smax(ssei(a),b); }
  ...
  __forceinline int reduce_min(const ssei& v) { return smin(v[0],v[1],v[2],v[3]); }
  __forceinline int reduce_max(const ssei& v) { return smax(v[0],v[1],v[2],v[3]); }
